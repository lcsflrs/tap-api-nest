...
> {
  ...
  async execute(command: BuyIngressCommand): Promise<BuyIngressResult> {
    ...
    if (!ingressBatch) {
      throw new Error("Ingress batch not found");
    }
  ...
  private async handlePromoterBonus(
    ...
    if (confirmedInvites === PROMOTER_FREE_INGRESS_THRESHOLD) {
      ...
      const promoterAlreadyHasIngress =
        await this.ingressRepository.findActiveByCustomerAndParty(
          invitedByCustomerId,
          partyId,
        );

      if (promoterAlreadyHasIngress) {
        return;
      }
      ...
      await this.ingressRepository.create(
        Ingress.create(
          new IngressID(0),
          new PartyID(partyDomainId),
          new IngressStatusID(2),
          5,
